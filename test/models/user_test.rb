...
class UserTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @user = User.create(name: "Bill Rodges", email: "billr@example.com",
  ...
  test "name should be 50 characters maximum" do 
    @user.name = "a" * 51
    assert_not @user.valid?
  end
  ...
  test "email should be 256 chars maximum" do
  ...
  test "email validation should accept valid addresses" do 
    valid_addresses = %w[user@example.com USER@foo.COM A_US-ER@foor.bar.org
                         first.last@foo.jp alice+bob@baz.fi]
